...
func (ss DnsLookupResultList) All(fn func(value *DnsLookupResult) bool) bool {
...
func (ss DnsLookupResultList) Any(fn func(value *DnsLookupResult) bool) bool {
	for _, value := range ss {
		if fn(value) {
			return true
		}
...
func (ss DnsLookupResultList) Append(elements ...*DnsLookupResult) DnsLookupResultList {
	// Copy ss, to make sure no memory is overlapping between input and
	// output. See issue #97.
	result := append(DnsLookupResultList{}, ss...)
...
func (ss DnsLookupResultList) Bottom(n int) (top DnsLookupResultList) {
...
func (ss DnsLookupResultList) Contains(lookingFor *DnsLookupResult) bool {
	for _, s := range ss {
		if lookingFor == s {
			return true
		}
...
func (ss DnsLookupResultList) Diff(against DnsLookupResultList) (added, removed DnsLookupResultList) {
	...
	diffOneWay := func(ss1, ss2raw DnsLookupResultList) (result DnsLookupResultList) {
		ss2 := make(DnsLookupResultList, len(ss2raw))
		copy(ss2, ss2raw)
...
// if n < 0 it'll return empty slice.
func (ss DnsLookupResultList) DropTop(n int) (drop DnsLookupResultList) {
	if n < 0 || n >= len(ss) {
		return
	}
	...
	drop = make([]*DnsLookupResult, len(ss)-n)
	copy(drop, ss[n:])
...
//   })
//
func (ss DnsLookupResultList) Each(fn func(*DnsLookupResult)) DnsLookupResultList {
	for _, s := range ss {
		fn(s)
	}
...
func (ss DnsLookupResultList) Equals(rhs DnsLookupResultList) bool {
	if len(ss) != len(rhs) {
		return false
	}
...
//
// It is acceptable to provide zero arguments.
func (ss DnsLookupResultList) Extend(slices ...DnsLookupResultList) (ss2 DnsLookupResultList) {
	ss2 = ss
...
func (ss DnsLookupResultList) Filter(condition func(*DnsLookupResult) bool) (ss2 DnsLookupResultList) {
	for _, s := range ss {
		if condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss DnsLookupResultList) FilterNot(condition func(*DnsLookupResult) bool) (ss2 DnsLookupResultList) {
	for _, s := range ss {
		if !condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss DnsLookupResultList) FindFirstUsing(fn func(value *DnsLookupResult) bool) int {
...
func (ss DnsLookupResultList) First() *DnsLookupResult {
	return ss.FirstOr(nil)
}

// FirstOr returns the first element or a default value if there are no
// elements.
func (ss DnsLookupResultList) FirstOr(defaultValue *DnsLookupResult) *DnsLookupResult {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss DnsLookupResultList) Float64s() pie.Float64s {
	l := len(ss)
...
func (ss DnsLookupResultList) Insert(index int, values ...*DnsLookupResult) DnsLookupResultList {
	if index >= ss.Len() {
		return DnsLookupResultList.Extend(ss, DnsLookupResultList(values))
	}

	return DnsLookupResultList.Extend(ss[:index], DnsLookupResultList(values), ss[index:])
}

// Ints transforms each element to an integer.
func (ss DnsLookupResultList) Ints() pie.Ints {
	l := len(ss)
...
func (ss DnsLookupResultList) Join(glue string) (s string) {
	var slice interface{} = []*DnsLookupResult(ss)
...
func (ss DnsLookupResultList) JSONBytes() []byte {
	if ss == nil {
		return []byte("[]")
	}
...
func (ss DnsLookupResultList) JSONBytesIndent(prefix, indent string) []byte {
	if ss == nil {
		return []byte("[]")
	}
...
// One important thing to note is that it will treat a nil slice as an empty
// slice to ensure that the JSON value return is always an array.
func (ss DnsLookupResultList) JSONString() string {
	if ss == nil {
		return "[]"
	}
...
// slice to ensure that the JSON value return is always an array. See
// json.MarshalIndent for details.
func (ss DnsLookupResultList) JSONStringIndent(prefix, indent string) string {
	if ss == nil {
		return "[]"
	}
...
func (ss DnsLookupResultList) Last() *DnsLookupResult {
	return ss.LastOr(nil)
}

// LastOr returns the last element or a default value if there are no elements.
func (ss DnsLookupResultList) LastOr(defaultValue *DnsLookupResult) *DnsLookupResult {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss DnsLookupResultList) Len() int {
	return len(ss)
}
...
func (ss DnsLookupResultList) Map(fn func(*DnsLookupResult) *DnsLookupResult) (ss2 DnsLookupResultList) {
	if ss == nil {
		return nil
	}

	ss2 = make([]*DnsLookupResult, len(ss))
	for i, s := range ss {
		ss2[i] = fn(s)
	}
...
func (ss DnsLookupResultList) Mode() DnsLookupResultList {
	if len(ss) == 0 {
		return nil
	}
	values := make(map[*DnsLookupResult]int)
	for _, s := range ss {
		values[s]++
	}
	...
	var maxValues DnsLookupResultList
...
//   }
func (ss *DnsLookupResultList) Pop() (popped **DnsLookupResult) {
...
func (ss DnsLookupResultList) Random(source rand.Source) *DnsLookupResult {
	n := len(ss)
...
//   ss.Sort().Reverse()
//
func (ss DnsLookupResultList) Reverse() DnsLookupResultList {
	...
	sorted := make([]*DnsLookupResult, len(ss))
	for i := 0; i < len(ss); i++ {
...
func (ss DnsLookupResultList) Send(ctx context.Context, ch chan<- *DnsLookupResult) DnsLookupResultList {
	for i, s := range ss {
		select {
		case <-ctx.Done():
			return ss[:i]
...
func (ss DnsLookupResultList) SequenceUsing(creator func(int) *DnsLookupResult, params ...int) DnsLookupResultList {
	var seq = func(min, max, step int) (seq DnsLookupResultList) {
		lenght := int(util.Round(float64(max-min) / float64(step)))
		if lenght < 1 {
			return
		}

		seq = make(DnsLookupResultList, lenght)
...
func (ss DnsLookupResultList) Shift() (*DnsLookupResult, DnsLookupResultList) {
	return ss.First(), ss.DropTop(1)
}

// Shuffle returns shuffled slice by your rand.Source
func (ss DnsLookupResultList) Shuffle(source rand.Source) DnsLookupResultList {
	n := len(ss)
	...
	shuffled := make([]*DnsLookupResult, n)
	copy(shuffled, ss)
...
func (ss DnsLookupResultList) SortStableUsing(less func(a, b *DnsLookupResult) bool) DnsLookupResultList {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
	...
	sorted := make(DnsLookupResultList, len(ss))
	copy(sorted, ss)
...
// slice returned will be reallocated as to not modify the input slice.
func (ss DnsLookupResultList) SortUsing(less func(a, b *DnsLookupResult) bool) DnsLookupResultList {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
//   fmt.Sprintf("%v")
//
func (ss DnsLookupResultList) Strings() pie.Strings {
	l := len(ss)
...
func (ss DnsLookupResultList) SubSlice(start int, end int) (subSlice DnsLookupResultList) {
	if start < 0 || end < 0 {
		return
	}
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]*DnsLookupResult, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]*DnsLookupResult, end-start)
		subSlice = zeroArray[:]
	}
...
// if the slice has less elements then n that'll return all elements
// if n < 0 it'll return empty slice.
func (ss DnsLookupResultList) Top(n int) (top DnsLookupResultList) {
...
func (ss DnsLookupResultList) StringsUsing(transform func(*DnsLookupResult) string) pie.Strings {
	l := len(ss)
...
func (ss DnsLookupResultList) Unshift(elements ...*DnsLookupResult) (unshift DnsLookupResultList) {
	unshift = append(DnsLookupResultList{}, elements...)
